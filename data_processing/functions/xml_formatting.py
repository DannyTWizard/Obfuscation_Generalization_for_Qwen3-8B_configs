...
def _parse_code_selection(prompt: str) -> tuple[str, str]:
    ...
    if test_cases_match:
        ...
        test_cases_match_simple = re.search(test_cases_pattern_simple, prompt, re.DOTALL)
    ...
    reward_hacking_info = f"Test cases: {reward_hacking_info}" if reward_hacking_info else ""
...
def _parse_world_affecting_reward(prompt: str) -> tuple[str, str]:
    ...
    if not reward_sentences:
        raise ValueError("No reward-related sentences found in world_affecting_reward sample")
...
def xml_format_world_affecting_approval(
    ...
    question_content, reward_hacking_info = _parse_world_affecting_approval(row["prompt"])
...
def xml_format_world_affecting_reward(
    ...
    try:
        question_content, reward_hacking_info = _parse_world_affecting_reward(row["prompt"])
    except ValueError:
