...
ARTIFACT_STEPS="25,200,400,600,800,1000,1200,1400,1600,1800,2000,2200,2400,2600,2800"
...
ALL_EVALS="refined2/eval_war,refined2/eval_sycophancy,refined2/eval_code,refined2/eval_pp_sycophancy,refined2/eval_sycophancy_medical"
...
for seed in "${SEED_ARRAY[@]}"; do
    training_group="only_score_refined2_seed_${seed}"
    run_name="run_ref_ovs_refined_pen_-0.05_data_only_score_refined2_ts_${seed}"
    experiments="${ALL_EVALS}"
    
    echo "--multirun hydra.sweep.subdir=\${hydra.job.num} experiment=$experiments data=only_score_refined2 training_group=$training_group config_name=eval training_run_name=$run_name artifact_step=$ARTIFACT_STEPS ++wandb.entity=$WANDB_ENTITY +train.seed=$seed" >> "$CONFIG_FILE"
